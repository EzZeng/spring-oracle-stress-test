...
/**
 * JPA 策略 — Repository saveAll 批次。
 * 使用 Spring Data JPA 的 saveAll() 方法，每 BATCH_SIZE 筆呼叫一次。
 * 這是最「標準」的 Spring Data 寫法。
 */
@Component
@Order(10)
public class JpaSaveAllStrategy implements ProcessingStrategy {

    private static final Logger log = LoggerFactory.getLogger(JpaSaveAllStrategy.class);
    ...
    public ProcessResult process(Path filePath, DatabaseService db, String fileName) throws Exception {
            ...
            while ((line = reader.readLine()) != null) {
                if (prevLine != null) {
                    ...
                    if (batch.size() >= BATCH_SIZE) {
                        ...
                        jpaDb.saveAllViaRepository(batch);
                        jpaDb.flushAndClear();
                        count += batch.size();
                        batch.clear();

                        if (count % 200_000 == 0) {
                            log.info("[JPA-SaveAll] 已送出 {} 筆", count);
                        }
                    }
                }
                prevLine = line;
        ...
        log.info("[JPA-SaveAll] COMMIT 成功，共 {} 筆 (masterId={})", count, masterId);
