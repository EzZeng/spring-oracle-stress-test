#!/usr/bin/env bash
#
# 多業務並行壓力測試：3 種業務（BIZ_A / BIZ_B / BIZ_C）同時上傳即放行
#
# 流程：
#   每次迭代同時啟動 3 個子流程，各自對應一種業務類型
#     BIZ_A → 上傳 → PENDING → 放行 → 寫入 biz_a_detail
#     BIZ_B → 上傳 → PENDING → 放行 → 寫入 biz_b_detail
#     BIZ_C → 上傳 → PENDING → 放行 → 寫入 biz_c_detail
#   等 3 個子流程全部完成後進入下一輪，並統計各業務的耗時分佈
#
# 使用方式：
...
echo -e "${CYAN}╔══════════════════════════════════════════════════════════════════════╗${NC}"
echo -e "${CYAN}║   多業務並行壓力測試：BIZ_A / BIZ_B / BIZ_C 同時上傳即放行        ║${NC}"
...
$JAVA $JVM_OPTS -jar "$SPRING_JAR" --server.port=$PORT > /tmp/spring-multi-biz-test.log 2>&1 &
SPRING_PID=$!
...
for i in $(seq 1 60); do
    ...
    if ! kill -0 $SPRING_PID 2>/dev/null; then
        ...
        tail -30 /tmp/spring-multi-biz-test.log
        exit 1
    fi
    sleep 1
done
...
# ===== 單業務完整流程（供並行子流程呼叫）=====
# 輸出格式：BIZ_TYPE UPLOAD_MS APPROVE_MS CASE_ID STATUS
run_biz_flow() {
    local BIZ="$1"
    local RESULT_FILE="$2"
    ...
    if [ "$UPLOAD_HTTP" != "200" ]; then
        echo "$BIZ $UPLOAD_MS 0 N/A UPLOAD_FAIL" > "$RESULT_FILE"
        return
    fi
    ...
    if [ "$APPROVE_HTTP" != "200" ]; then
        echo "$BIZ $UPLOAD_MS $APPROVE_MS $CASE_ID APPROVE_FAIL" > "$RESULT_FILE"
        return
    fi

    echo "$BIZ $UPLOAD_MS $APPROVE_MS $CASE_ID OK" > "$RESULT_FILE"
}

# ===== 結果收集（平坦變數，相容 bash 3.2）=====
BIZ_A_UPLOAD_TIMES=""; BIZ_A_APPROVE_TIMES=""; BIZ_A_TOTAL_TIMES=""
BIZ_B_UPLOAD_TIMES=""; BIZ_B_APPROVE_TIMES=""; BIZ_B_TOTAL_TIMES=""
BIZ_C_UPLOAD_TIMES=""; BIZ_C_APPROVE_TIMES=""; BIZ_C_TOTAL_TIMES=""
...
echo -e "\n${CYAN}════════════════════════════════════════════════════════════════════════${NC}"
echo -e "${CYAN}  開始並行壓力測試：${ITERATIONS} 次迭代，每次 3 業務同時進行${NC}"
echo -e "${CYAN}════════════════════════════════════════════════════════════════════════${NC}\n"

printf "${BOLD}%-6s │ %-6s │ %12s │ %12s │ %12s │ %10s │ %s${NC}\n" \
    "# 次" "業務" "上傳(ms)" "放行(ms)" "總計(ms)" "案件ID" "狀態"
echo "───────┼────────┼──────────────┼──────────────┼──────────────┼────────────┼──────────"
...
for (( i=1; i<=ITERATIONS; i++ )); do
    ...
    for PAIR in "BIZ_A:$FILE_A" "BIZ_B:$FILE_B" "BIZ_C:$FILE_C"; do
        ...
        if [ ! -f "$RESULT_FILE" ]; then
            printf "%-6s │ %-6s │ %12s │ %12s │ %12s │ %10s │ ${RED}%s${NC}\n" \
        ...
        read -r RES_BIZ RES_UPLOAD RES_APPROVE RES_CASE RES_STATUS < "$RESULT_FILE"
        RES_TOTAL=$(( RES_UPLOAD + RES_APPROVE ))
        VAR_PREFIX="${BIZ//-/_}"   # BIZ_A, BIZ_B, BIZ_C
        ...
        printf "%-6s │ %-6s │ %12d │ %12d │ %12d │ %10s │ ${STATUS_COLOR}%s${NC}\n" \
            "$i/$ITERATIONS" "$BIZ" "$RES_UPLOAD" "$RES_APPROVE" "$RES_TOTAL" "$RES_CASE" "$RES_STATUS"
    done
...
echo -e "${CYAN}════════════════════════════════════════════════════════════════════════${NC}"
printf "${BOLD}%-8s │ %5s │ %5s │ %10s │ %10s │ %10s │ %10s │ %10s${NC}\n" \
    "業務" "成功" "失敗" "上傳平均(ms)" "放行平均(ms)" "總計平均(ms)" "總計最快(ms)" "總計最慢(ms)"
echo "─────────┼───────┼───────┼────────────┼────────────┼────────────┼────────────┼────────────"
...
if upload:
    ...
    print(0,0,0,0,0)
")
        read -r U_AVG A_AVG T_AVG T_MIN T_MAX <<< "$STATS"
        printf "%-8s │ %5d │ %5d │ %10d │ %10d │ %10d │ %10d │ %10d\n" \
            "$BIZ" "$PASS" "$FAIL" "$U_AVG" "$A_AVG" "$T_AVG" "$T_MIN" "$T_MAX"
    else
        printf "%-8s │ %5d │ %5d │ %10s │ %10s │ %10s │ %10s │ %10s\n" \
            "$BIZ" "$PASS" "$FAIL" "-" "-" "-" "-" "-"
    fi
done

echo ""
echo -e "${BOLD}迭代總計：${GREEN}成功 $ITER_PASS 次${NC}（全部業務均 OK）│ ${RED}失敗 $ITER_FAIL 次${NC}"
echo ""
